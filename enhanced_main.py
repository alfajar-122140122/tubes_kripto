...
This is an enhanced version of the main entry point for the QR Code encryption and steganography application.
It integrates RSA encryption, QR code generation, and improved DCT-based steganography.
"""
...
from src.enhanced_qr_code import EnhancedQRCodeProcessor
from src.enhanced_dct_steganography import EnhancedDCTSteganography
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Enhanced QR Code Encryption and Steganography Using RSA and DCT'
    )
    ...
    extract_parser.add_argument('--output-dir', type=str, default='output',
                             help='Directory to save output files')
    extract_parser.add_argument('--use-map', action='store_true',
                              help='Use embedding map for extraction if available')
    ...
    decrypt_parser.add_argument('--output-dir', type=str, default='output',
                              help='Directory to save output files')
    decrypt_parser.add_argument('--save-processed', action='store_true',
                              help='Save processed images during QR detection')
    ...
    reverse_parser.add_argument('--output-dir', type=str, default='output',
                              help='Directory to save output files')
    reverse_parser.add_argument('--save-processed', action='store_true',
                              help='Save processed images during QR detection')
    reverse_parser.add_argument('--use-map', action='store_true',
                              help='Use embedding map for extraction if available')
...
def encrypt_and_generate_qr(args):
    ...
    # Generate QR code with enhanced processor
    qr_processor = EnhancedQRCodeProcessor()
...
def hide_qr_in_image(args):
    """
    Hide QR code in a cover image using enhanced DCT steganography
    """
    print("Hiding QR code in cover image with enhanced steganography...")
    ...
    dct_steg = EnhancedDCTSteganography()
...
def extract_qr_from_image(args):
    """
    Extract QR code from steganographic image using enhanced methods
    """
    print("Extracting QR code from steganographic image with enhanced extraction...")
    ...
    output_path = dct_steg.extract(
        ...
        use_map=args.use_map if hasattr(args, 'use_map') else False
    )
    
    print(f"Extracted QR code saved to: {output_path}")
    
    # Also save denoised and sharpened versions
    base_name = os.path.splitext(args.extracted_output)[0]
    ...
    if os.path.exists(os.path.join(output_dir, f"{base_name}_denoised.png")):
        print(f"Denoised version saved to: {denoised_path}")
        print(f"Sharpened version saved to: {sharpened_path}")
    
    return output_path

def decrypt_qr_code(args):
    """
    Read QR code and decrypt data with enhanced QR detection
    """
    print("Reading QR code and decrypting data with enhanced detection...")
    ...
    if hasattr(args, 'save_processed') and args.save_processed:
        save_processed = True
        processed_dir = os.path.join(args.output_dir, "processed_qr")
        ensure_directory_exists(processed_dir)
    
    # Read QR code with enhanced processor
    qr_processor = EnhancedQRCodeProcessor()
    encrypted_data = qr_processor.read_qr_code(
        args.qr_path,
...
def execute_pipeline(args):
    ...
    """
    print("Executing full encryption and hiding pipeline with enhanced algorithms...")
...
def execute_reverse_pipeline(args):
    ...
    """
    print("Executing full extraction and decryption pipeline with enhanced algorithms...")
    ...
    decrypt_args = argparse.Namespace(
        ...
        output_dir=args.output_dir,
        save_processed=args.save_processed if hasattr(args, 'save_processed') else False
    )
    decrypted_data = decrypt_qr_code(decrypt_args)
    
    # Also try with processed versions if decryption failed
    if decrypted_data is None:
        print("Initial decryption failed. Attempting with processed versions...")
        
        # Try with denoised version
        denoised_path = os.path.join(args.output_dir, "extracted_qrcode_denoised.png")
        if os.path.exists(denoised_path):
            print("Trying with denoised version...")
            decrypt_args.qr_path = denoised_path
            decrypted_data = decrypt_qr_code(decrypt_args)
        
        # Try with sharpened version if still no success
        if decrypted_data is None:
            sharpened_path = os.path.join(args.output_dir, "extracted_qrcode_sharpened.png")
            if os.path.exists(sharpened_path):
                print("Trying with sharpened version...")
                decrypt_args.qr_path = sharpened_path
                decrypted_data = decrypt_qr_code(decrypt_args)
    
    print("Reverse pipeline completed!")
    print(f"Extracted QR code: {extracted_qr_path}")
    if decrypted_data:
        print(f"Decrypted data: {decrypted_data}")
    else:
        print("Decryption failed. The QR code could not be read or decoded properly.")
